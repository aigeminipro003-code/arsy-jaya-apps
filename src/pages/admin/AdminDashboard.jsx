...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        async function load() {
            ...
            const [{ data: mat }, { data: logs }, { data: movements }, { data: adjustments }] = await Promise.all([
                ...
                supabase.from('stock_movements')
                    ...
                    .order('created_at', { ascending: false })
                    .limit(20),
                supabase.from('app_settings').select('value').eq('key', 'daily_target_m').maybeSingle()
            ])

            const matList = mat || []
            const logList = logs || []
            const mvList = movements || []
            const settingsData = settings || null
    ...
    return (
        ...
        <div className="animate-fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                ...
                <button 
                    onClick={() => { setNewTarget(dailyTarget.toString()); setShowSettings(true); }}
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Bahan Kritis ({"<"}10m)</span>
                        </div>
                    ...
                    <div style={{ padding: '8px 0' }}>
                        {lowStockMaterials.length > 0 ? (
                                    ...
                                    <div key={m.id} style={{
                                        ...
                                        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    // Mock Interaction (To act as Trigger UI)
                                                    alert(`[Mock] Notifikasi Pemesanan untuk bahan ${m.name} terkirim.`)
                                                }}
                                            >
                                                Pesan Lagi
                                            </button>
                            ...
                            <div style={{ padding: '32px 24px', textAlign: 'center', fontSize: 13, color: '#22c55e', fontWeight: 600 }}>
                                ✓ Stok semua bahan di atas 10 meter.
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div style={{ padding: 20 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <input 
                                    ...
                                    style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-bg)', color: 'var(--color-text-primary)', fontSize: 14 }}
                                />
                                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-muted)' }}>meter</span>
                            </div>
                        ...
                        <div style={{ padding: '16px 20px', borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    const { error } = await supabase.from('app_settings').upsert({ key: 'daily_target_m', value: val.toString() })
